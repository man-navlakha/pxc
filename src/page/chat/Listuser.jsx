...
// --- NEW: UI Components for different states ---
...
const EmptyState = ({ onFindFriendsClick }) => (
    <div className="text-center py-20 text-white/60">
        <span className="material-symbols-outlined text-6xl text-white/20">chat_bubble</span>
        ...
        <button onClick={onFindFriendsClick} className="mt-6 bg-white/10 hover:bg-white/20 rounded-lg py-2 px-4 text-white/80 text-sm font-semibold transition-colors">
            Find Friends
        </button>
    </div>
);
...
export default function Listuser() {
    ...
    // --- All of your existing logic is preserved below ---
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            const deduplicated = Array.from(new Map(combined.map((u) => [u.username, u])).values());
            ...
            updated.sort((a, b) => new Date(b.lastTime || 0) - new Date(a.lastTime || 0));
            setAllUsers(updated);
        } catch (err) {
            ...
            <main className="flex-1 flex flex-col p-4">
                {loading ? (
                    <div className="space-y-2">
                        {Array.from({ length: 5 }).map((_, i) => <ChatUserSkeleton key={i} />)}
                    </div>
                ) : allUsers.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-center">
                                                <span className="font-semibold truncate flex justify-center items-center gap-1 ">{user.username} {verifiedUsernames.has(user?.username) && <VerifiedBadge size={24} />}</span>
                                                
                                                {user.lastTime && (
                                                    <span className="text-xs text-white/40 flex-shrink-0 ml-2">
                                                        {new Date(toISOStringCompat(user.lastTime)).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                                                    </span>
                                                )}
                                            </div>
                                           <p className={`text-sm truncate max-w-36 ${user.lastSender !== USERNAME && !user.isSeen ? 'text-white font-semibold' : 'text-white/60'}`}>
