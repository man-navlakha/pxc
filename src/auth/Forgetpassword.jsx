...
const Forgetpassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post("user/password_reset/", { email });
  ...
  const startPolling = (emailToPoll) => {
    pollingInterval.current = setInterval(async () => {
      try {
        const response = await api.post("user/reset/status/", { email: emailToPoll });
        ...
        console.error("Error while polling:", err.response?.data || err.message);
      }
          ...
          {loading ? (
            ...
            <div className="flex items-center flex-col justify-center w-[360px] p-6">
              <div className=" px-4 py-6 flex flex-col bg-white border shadow-lg border-gray-200 max-w-[360px] w-full max-h-screen rounded-xl ">
                ...
                {successMessage && (
                  <p className="text-green-600 text-center font-bold m-2 text-md">{successMessage}</p>
                )}
                <h1 className="text-xl mb-3 font-bold">Forget your password</h1>
                <div>
                  <form
                    onSubmit={handleSubmit}
                    className={`flex flex-col gap-3 ${loading ? "hidden" : ""}`}
                  >
              ...
              <p className="mt-4 text-gray-400 dark:text-gray-600 font-normal text-sm text-center">
                I don't want to Forget?{" "}
                <Link
                  className="text-gray-300 hover:text-gray-1k hover:underline font-semibold transition-all ease-in-out"
                  to="/auth/login"
                >
                  Log in
                </Link>
